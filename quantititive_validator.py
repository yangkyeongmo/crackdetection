...
def _get_area_comparison(expected_ovl, real_ovl,
                         x, next_x, y, next_y, val_img):
    seg = next_x - x
    fp, fn = 0, 0
    core_x, core_y = x + seg//2, y + seg//2

    if all(expected_ovl[_y, _x][2] > 0
           for _y in range(y, next_y) for _x in range(x, next_x)):
        if not any(real_ovl[_y, _x][2] > 0 for _y in range(y, next_y)
                   for _x in range(x, next_x)):
            val_img[y:next_y, x:next_x] = (255, 0, 0)
            fp = 1
    else:
        if any(real_ovl[_y, _x][2] > 0 for _y in range(core_y, core_y + 1)
               for _x in range(core_x, core_x + 1)):
...
def _iterate_through_imgs_areas(expected_ovl_path, real_ovl_path, seg):
    ...
    while next_x < w:
        ...
        while next_y < h:
            ...
            y, next_y = y + move, next_y + move
        x, next_x = x + move, next_x + move
...
def _get_image_comparison_result(expected_ovl_path, real_ovl_path, seg):
    _fp, _fn = 0, 0
    _fp, _fn = _iterate_through_imgs_areas(expected_ovl_path,
                                           real_ovl_path, seg)
