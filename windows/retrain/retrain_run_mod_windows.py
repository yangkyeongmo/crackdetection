...
parent_dataPath =\
    r'C:\Users\HP\Dropbox\Projects\crack\data\output\train_result'
...
def run_segs(filepath, arg):
    ...
    while x < w:
        while y < h:
            ...
            cv2.imwrite(arg.segname + '.jpg', segment)
            prediction = run_inference_on_image(
                    arg.segname + '.jpg',
                    sess,
    ...
    # write result
    rsltpath = os.path.join(dest,
                            path[-6:-4] + '_' +
                            path_under_path.replace("\\", "_") +
                            ',' + seg.__str__() + '.jpg')
